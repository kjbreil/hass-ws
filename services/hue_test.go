...
func TestHueActivateScene_JSON(t *testing.T) {
	...
	tests := []struct {
		...
		fields: NewHueActivateScene(Targets("climate.kitchen")).Brightness(brightness).Speed(speed).Transition(transition),
		name:   "base",
...
func TestHueHueActivateScene_JSON(t *testing.T) {
	...
	tests := []struct {
		...
		fields: NewHueHueActivateScene(Targets("climate.kitchen")).GroupName(groupName).SceneName(sceneName),
		name:   "base",
